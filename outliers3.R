...
final_model_cleaned <- lm(TUITION_FEE ~ poly(SOCIOECONOMIC_STATUS, 4) + 
                            poly(FATHER_INCOME, 1) + 
                            poly(MOTHER_INCOME, 2) + 
                            poly(MONTHLY_SCHOOL_FEE, 4) + PBM, data = cleaned_data)

# Display summary of cleaned model
summary(final_model_cleaned)

# Generate diagnostic plots for the cleaned model
...
plot(final_model_cleaned$fitted.values, final_model_cleaned$residuals, 
...
qqnorm(final_model_cleaned$residuals, main = "Normal Q-Q Plot")
qqline(final_model_cleaned$residuals, col = "red", lwd = 2)

# Scale-Location Plot (Variance Homogeneity)
plot(final_model_cleaned$fitted.values, sqrt(abs(final_model_cleaned$residuals)), 
...
plot(cooks.distance(final_model_cleaned), type = "h", 
